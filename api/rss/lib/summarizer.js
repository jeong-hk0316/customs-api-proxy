...
function summarizeKo20(text, maxLength = 100) {
  ...
  // 요약 전략
  if (keywords.length >= 6 && actionKeyword) {
    const mainKeywords = keywords.slice(0, 6).join(' ');
    const combined = `${mainKeywords} ${actionKeyword}`;
    summary = combined.length <= maxLength ? combined : keywords.slice(0, 5).join(' ') + ` ${actionKeyword}`;
